    ...
    public class Fahrzeug
    {
        ...
        public void Beschleunige(int a)
        {
            if (this.MotorLaeuft)
            {
                ...
                //if (AktGeschwindigkeit < 0) AktGeschwindigkeit = 0;
                //if(AktGeschwindigkeit > MaxGeschwindigkeit) AktGeschwindigkeit = MaxGeschwindigkeit;


                // Wenn Differenz + AktGeschwindigkeit > Max dann Akt auf Max setzen
                if (this.AktGeschwindigkeit + a > this.MaxGeschwindigkeit)
                    this.AktGeschwindigkeit = this.MaxGeschwindigkeit;
                // Wenn Differenz + AktGeschwindigkeit < 0 dann Akt auf 0 setzen
                else if (this.AktGeschwindigkeit + a < 0)
                    this.AktGeschwindigkeit = 0;
                else
                    // Ansonsten Akt um Differenz erhöhen (kann auch negativ sein)
